...
static TokenList TokenListCreate(unsigned int start_capacity) {
    ...
    ASSERT(malloc != NULL, "Failed to allocate memory for token list%c", ' '); // My shitty macro demands fancy string formatting
...
static void AddToken(char lexeme[LEXEME_MAX_LENGTH], TokenType type, TokenList* list) {
    if (list->length >= list->capacity) {
        list->capacity *= 2;
        ASSERT(realloc(list->data, list->capacity), "Failed to reallocate memory for token list%c", ' ');
    }
